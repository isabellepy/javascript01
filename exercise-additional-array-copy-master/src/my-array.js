...
function immutablePushNumber(source, number) {
  if(!Array.isArray(source)){
    throw new TypeError("You must enter an array on the first argument");
  } else if(isNaN(number)) {
    throw new TypeError("You must enter a number on the second argument");
  }
  let copyArray = source.slice();
